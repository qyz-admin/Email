...
// @downloadURL  https://qyz-admin.github.io/Email/FB-2021.4.js
// @updateURL    https://qyz-admin.github.io/Email/FB-2021.4.js
// @grant    none
// ==/UserScript==
...
//--------------------------------1、-检测是否有私讯
   function testCCC() { //正在刷新网页内容（三）
       var tags = document.getElementsByClassName("j83agx80 cbu4d94t ew0dbk1b irj2b8pg");
       var tagsValue = tags[15].firstElementChild.firstElementChild.innerText;
       var gengxin = 0;//0 表示没有；1表示无信息；2表示有信息需查看；
       var dongtai = 0;
       var shoujian = 0;
       var tongzhi = 0;
       for(var t=0; t<tags.length;t++){
            var val = tags[t].firstElementChild.firstElementChild.innerText;
            if (val =="更新内容") {
                gengxin = tags[t].childElementCount;
                console.log(val);
                break
            }else{
                gengxin = 0;
       ...
       console.log(gengxin);
        for(var t1=0; t1<tags.length;t1++){
            var val1 = tags[t1].firstElementChild.firstElementChild.innerText;
            if (val1=="动态消息") {
                dongtai = tags[t1].childElementCount;
                console.log(val1);
                break
            }else{
                dongtai = 0;
       ...
       console.log(shoujian);
       for(var t3=0; t3<tags.length;t3++){
            var val3 = tags[t3].firstElementChild.firstElementChild.innerText;
            if (val3=="通知") {
                tongzhi = tags[t3].childElementCount;
                       console.log(val3);
                break
            }else{
                tongzhi = 0;
       ...
       console.log(tongzhi);
       if(gengxin <= "1" && dongtai <= "1" && shoujian <= "1" && tongzhi <= "1"){
                 console.log("进行关闭网页");
                 window.opener = null;//禁止某些浏览器的一些弹窗
                 window.open('','_self');
                 window.close()
       }else if(gengxin <= "1" && dongtai <= "1" && shoujian == "2" && tongzhi <= "2"){
                    console.log("进行跳转网页");
 ...
 function testAAA() { //正在刷新网页内容（三）
       ...
       for(var t=0; t<tags.length;t++){//--1
        ...
        //--2  if (tags[t].firstElementChild.firstElementChild.innerText == "更新内容"){
            if (tags[t].firstElementChild.firstElementChild.innerText == "Inbox" || tags[t].firstElementChild.firstElementChild.innerText == "收件箱") {
             ...
             var rr = tags[t].parentNode.parentNode.childElementCount;
                var rr2 = tags[t-1].parentNode.parentNode.childElementCount;
             if (rr == "2"&& rr2 == "1") {
                    console.log("进行跳转网页");
                 var cod009=window.location.href;
                     cod009=cod009 +"inbox";
